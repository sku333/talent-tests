...
export default function Home(){
 ...
 const ranked=dims.map((d,i)=>({d,score:scores[i]})).sort((a,b)=>b.score-a.score),top=ranked.slice(0,3),low=ranked.slice(-2).reverse(),baseType=personas[top[0].d[0]][0],type=top[0].score-top[1].score<=2?`复合·${baseType}`:baseType,path=development[top[0].d[0]],careerDirections=top.flatMap(x=>x.d[6].map(name=>({name,dimension:x.d[1],reason:x.d[3]}))).slice(0,6);
 ...
 return <main className="result-page autumn">
  <nav><div className="brand small"><span className="brand-mark"><Leaf/></span>拾光天赋</div><button className="restart" onClick={restart}><RotateCcw/>重新测试</button></nav>
  <header className="result-hero reveal-section"><p className="part">PART 01 · 天赋人格</p><p className="eyebrow">YOUR TALENT ARCHETYPE</p><h1>你是<br/><em>「{type}」</em></h1><p className="persona-line">{personas[top[0].d[0]][1]}</p><p>你的优势链由 <b>{top[0].d[1]}</b> 发起，借助 <b>{top[1].d[1]}</b> 推进，再由 <b>{top[2].d[1]}</b> 放大。它不是唯一职业答案，而是一组可以迁移到不同领域的能力组合。</p></header>
  <section className="result-chapters">
   <section className="chapter reveal-section"><p className="part">PART 02 · 核心天赋</p><h2>三种力量，构成你的独特工作方式</h2><div className="top-three">{top.map((x,i)=><article key={x.d[0]}><span>核心天赋 {i+1}</span><b>{x.d[1]}</b><strong>{x.score}</strong><p>{x.d[3]}</p></article>)}</div></section>
   <section className="chapter reveal-section"><p className="part">PART 03 · 天赋组合</p><h2>{top[0].d[1]}负责启动，{top[1].d[1]}帮助推进</h2><p className="chapter-lead">当{top[2].d[1]}加入后，你不只是“擅长一件事”，而是更容易形成一条完整的优势链：发现重要线索、选择自然的处理方式，再把它转化为别人能够感受到的成果。</p><div className="combo-flow">{top.map((x,i)=><article key={x.d[0]}><span>0{i+1}</span><b>{x.d[1]}</b><small>{i===0?'启动与判断':i===1?'推进与连接':'呈现与放大'}</small></article>)}</div></section>
   <section className="chapter reveal-section"><p className="part">PART 04 · 能力全景</p><h2>你的十维天赋图谱</h2><div className="chart-layout"><Radar values={scores}/><div className="score-list">{dims.map((d,i)=><div key={d[0]}><span>{d[1]}</span><i><b style={{width:`${scores[i]}%`}}/></i><strong>{scores[i]}</strong></div>)}</div></div></section>
   <section className="chapter reveal-section"><p className="part">PART 05 · 十维详解</p><h2>不只看排名，也看你如何使用它</h2><div className="detail-grid">{ranked.map(x=><article key={x.d[0]}><header><b>{x.d[1]}</b><strong>{x.score}</strong></header><p>{x.d[3]}</p></article>)}</div></section>
   <section className="chapter reveal-section"><p className="part">PART 06 · 优势与盲点</p><h2>用好第一驱动力，也保护自己的边界</h2><div className="strength-grid"><article><Sparkles/><span>自然优势</span><p>{top[0].d[3]} 当它与{top[1].d[1]}结合，你更容易把零散线索变成可以推进的方向。</p></article><article><Target/><span>潜在盲点</span><p>{top[0].d[4]}</p></article></div></section>
   <section className="chapter reveal-section"><p className="part">PART 07 · 工作环境</p><h2>有些环境让你发光，有些只是让你更费力</h2><div className="strength-grid"><article><Heart/><span>更容易发光</span><p>{top[0].d[5]}</p></article><article><Compass/><span>隐性消耗</span><p>你的{low[0].d[1]}与{low[1].d[1]}相对不占优势。长期只依赖这些方式工作可能更费力，但可以通过团队互补、工具和流程降低消耗。</p></article></div></section>
   <section className="chapter reveal-section"><p className="part">PART 08 · 职业方向</p><h2>先选择工作内容，再选择职业名称</h2><div className="career-cards">{careerDirections.map((career,i)=>{const Icon=reportIcons[i];return <article key={`${career.name}-${i}`}><Icon/><div><span>{career.dimension}驱动</span><h3>{career.name}</h3><p>{career.reason}</p><small>入门验证：找一个真实小任务，连续体验三次，再判断是否值得投入。</small></div></article>})}</div><p className="note">推荐方向不是录取通知书。还需要结合兴趣、技能、价值观、现实机会与生活阶段判断。</p></section>
   <section className="chapter reveal-section"><p className="part">PART 09 · 发展路线</p><h2>让天赋从感受，长成别人看得见的能力</h2><div className="road-grid"><article><span>优先寻找</span><b>{path[0]}</b></article><article><span>起步验证</span><b>{path[1]}</b></article><article><span>长期积累</span><b>{path[2]}</b></article></div></section>
   <section className="blessing reveal-section"><p className="part">PART 10 · 写给你</p><Heart/><h2>愿你找到与灵魂契合的方向</h2><p>愿你不必成为别人眼中正确的人，而能慢慢靠近那个让自己感到舒展、笃定并充满生命力的方向。愿你的天赋被看见，热爱有处安放，付出的努力最终长成属于你的成功。</p><blockquote>你要寻找的，或许不是最喧闹的那条路，而是灵魂走上去以后，不再需要反复说服自己的那一条。</blockquote><button onClick={()=>setStage('home')}>回到首页</button></section>
  </section>
 </main>;
}
